...
We start with the anomalous magnetic moment of the muon since its parameter set only consists of $m$, $M_l$ and $g_2^l$. In general, every charged particle
...
from QED or hadronic vacuum polarisation but all of them are quantum loop effects. Even when all of them are taken into account, there is still a discrepancy
...
\begin{figure}[t]
 ...
 \caption{One-loop contribution to $a_\mu$. Depending on the model, there are multiple possibilites to attach the photon.}
 \label{pic_g-2}
\end{figure}
...
with
\begin{align}
 I(x) = \frac{1}{12(x-1)^4}\left(-5+5x-x^2+x^3+6x\log(x) \right).
\end{align}
...
prefactor $X$ of $O_\tau$  but one possibility is to regard the particles at each vertex as isospin multiplets. Since we want the vertices be 
...
one can write down using these operators. For the singlet case, this calculus can be done equivalently but can be skipped, because no charged currents as 
